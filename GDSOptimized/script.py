...
def main():
	...
	for info in finalList:
		...
		result=commands.getoutput('/usr/bin/python %s/main.py %s %s %d %d %d' % (path, "/storage/matur/querys/queryLog.txt", "/storage/matur/costos/tiemposSegados.txt", int(info[0]), int(info[1]), int(info[2])))
		print result
		c += 1
